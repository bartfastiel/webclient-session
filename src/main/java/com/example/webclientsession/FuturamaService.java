...
public class FuturamaService {
    ...
    FuturamaCharacter getFuturamaCharacter() {
        ResponseEntity<FuturamaCharacter> responseEntity = webClient.get()
                .uri("characters/2")
                .retrieve()
                .toEntity(FuturamaCharacter.class)
                .block();

        FuturamaCharacter character = Objects.requireNonNull(responseEntity).getBody();

        return new FuturamaCharacter(
                character.age(),
                character.gender(),
                character.id() + 17,
                character.nameInformation()
        );
    }
}
